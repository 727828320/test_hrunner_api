# NOTE: Generated By HttpRunner v3.1.5
# FROM: testcases\demo_testcase_ref.yml


import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent))


from httprunner import HttpRunner, Config, Step, RunRequest ,RunTestCase
from testcases.login_testcase import TestCaseLoginTestcaseRef as login


class TestCaseDepartmentCreateNameTestcaseRef(HttpRunner):

    config = (
        Config("request methods testcase: reference testcase")
        .base_url("https://qyapi.weixin.qq.com")
        .verify(False)
    )

    teststeps = [
        Step(
            RunTestCase("获取access_token")
            .with_variables()
            .call(login)
        ),
        Step(
            RunRequest("创建部门")
                .post("/cgi-bin/department/create")
                .with_params(**{"access_token": "$access_token"})
                .with_json({
                    "name_en": "${department_data(name_en)}",
                    "parentid": "${department_data(parentid)}",
                    "order": "${department_data(order)}",
                    "id": "${department_data(id)}"
            })
                .validate()
                .assert_equal("status_code", 200)
                .assert_equal("body.errcode", 60001)
        )
    ]

if __name__ == "__main__":
    TestCaseDepartmentCreateNameTestcaseRef().test_start()
